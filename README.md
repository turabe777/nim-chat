...
- **🤖 モデル選択**: 利用可能モデルからの選択
- **📊 埋め込み方式選択**: ローカル vs NVIDIA NIM API
...
1. **ローカル処理（デフォルト）**
   ...
   - **次元**: 384次元
   - **処理場所**: Apple Silicon GPU
   - **メリット**: 無料、高速、プライバシー保護
   
2. **NVIDIA NIM API**
   - **モデル**: `NV-Embed-QA`、`nvidia/nv-embedqa-e5-v5`
   - **次元**: モデルにより異なる（自動検出）
   - **処理場所**: NIMサーバー
   - **メリット**: 最新モデル、統一されたNIMエコシステム

### 設定方法
**WebUIから設定**:
1. サイドバーの「📊 埋め込み設定」
2. 「埋め込み方式」でローカル/NIM APIを選択
3. NIM使用時は「NIM埋め込みモデル」を選択
4. RAGシステムを再初期化

**注意事項**:
- NIM埋め込み使用にはNIMエンドポイントが必要
- 埋め込み方式変更時はシステム再初期化が必要
- 異なる次元のベクトルは自動的に処理
